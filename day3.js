...
let wiresRaw = input.split('\n')
let wires = wiresRaw.map(wire => wire.split(','))
let wire0position = [[0, 0]]
let position = [0, 0]
let step = 1
for (let i = 0; i < wires[0].length; i++) {
  const instruction = wires[0][i].charAt(0)
  const amount = parseInt(wires[0][i].substring(1))
  for (let j = 0; j < amount; j++) {
    ...
    wire0position.push([position[0], position[1], step++])
  }
}
...
for (let i = 0; i < wires[1].length; i++) {
  const instruction = wires[1][i].charAt(0)
  const amount = parseInt(wires[1][i].substring(1))
  for (let j = 0; j < amount; j++) {
    switch (instruction) {
      case 'R':
        position[0] += 1
    ...
    wire1position.push([position[0], position[1], step++])
  }
}
let intersections = []
wire0position.forEach(position0 => {
  wire1position.forEach(position1 => {
    if (position0[0] === position1[0] && position0[1] === position1[1]) {
      if (position0[0] !== 0 || position0[1] !== 0) {
        intersections.push({
          position: position0,
          distance: calcDistance(position0),
          steps: position0[2] + position1[2]
        })
      }
    }
  })
...
const minDistance = intersections.reduce(
  (min, p) => (p.distance < min ? p.distance : min),
  intersections[0].distance
)
console.log('minimum distance :', minDistance)
console.log(intersections.find(i => i.distance === minDistance))


const minSteps = intersections.reduce(
  (min, p) => (p.steps < min ? p.steps : min),
  intersections[0].steps
)
console.log('minimum distance :', minSteps)
console.log(intersections.find(i => i.steps === minSteps))
console.log('end')
